            ...
            public boolean addComponentParts(World worldIn, Random randomIn, StructureBoundingBox structureBoundingBoxIn)
            {
                ...
                if (!this.hasSpawner)
                {
                    ...
                    if (structureBoundingBoxIn.isVecInside(blockpos))
                    {
                        ...
                        {
                        	EventDungeon pre = new EventDungeon.Pre(tileentity, blockpos, EventDungeon.Type.NETHERFORTRESS);
                        	MinecraftForge.EVENT_BUS.post(pre);
                        	if(!pre.isCanceled())
                        		((TileEntityMobSpawner)tileentity).getSpawnerBaseLogic().setEntityId(DungeonMobs.pickMobSpawner(randomIn, Type.NETHERFORTRESS) );
                            EventDungeon post = new EventDungeon.Post(tileentity, blockpos, EventDungeon.Type.NETHERFORTRESS);
                        	MinecraftForge.EVENT_BUS.post(pre);
                        }
                    }
                }
