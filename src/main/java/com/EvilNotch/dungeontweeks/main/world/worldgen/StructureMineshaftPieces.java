                    ...
                    for (int j1 = 0; j1 < this.sectionCount; ++j1)
                    {
                        ...
                        if (this.hasSpiders && !this.spawnerPlaced)
                        {
                            ...
                            if (structureBoundingBoxIn.isVecInside(blockpos) && this.getSkyBrightness(worldIn, 1, 0, i2, structureBoundingBoxIn) < 8)
                            {
                                ...
                                {
                                	EventDungeon pre = new EventDungeon.Pre(tileentity, blockpos, EventDungeon.Type.MINESHAFT);
                                	MinecraftForge.EVENT_BUS.post(pre);
                                	if(!pre.isCanceled())
                                		((TileEntityMobSpawner)tileentity).getSpawnerBaseLogic().setEntityId(DungeonMobs.pickMobSpawner(randomIn, Type.MINESHAFT));
                                	EventDungeon post = new EventDungeon.Post(tileentity, blockpos, EventDungeon.Type.MINESHAFT);
